...
compile_wiki.py
---------------
Reads all raw source notes and uses Claude to synthesize a structured,
...
The compiler explicitly:
  ...
  - Generates insights not stated in any single raw note
...
Usage:
    ...
    python compile_wiki.py --dry-run    # show what would be compiled, don't call API
"""
...
# Each entry: (wiki_topic_name, short_description)
WIKI_TOPICS = [
    ("ExperienceMatching",
...
def compile_article(topic: str, description: str, sources_text: str) -> str:
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Show plan, don't call API")
    ...
    print(f"Loaded {len(raw)} files ({total_chars:,} chars)\n")

    topics_to_run = WIKI_TOPICS
    if args.topic:
        topics_to_run = [(t, d) for t, d in WIKI_TOPICS if t == args.topic]
        if not topics_to_run:
            valid = [t for t, _ in WIKI_TOPICS]
            print(f"Unknown topic. Valid: {valid}")
            return
    ...
    for topic, description in topics_to_run:
        ...
        article = compile_article(topic, description, sources_text)
    ...
    if compiled:
        ...
        for t, m in index.items():
            print(f"  {t:<33} {m['word_count']:>5}  {len(m['outgoing_links']):>4}  "
                  f"{len(m['incoming_links']):>4}")
